    ...
    try {
        ...
        if (!recipeId) {
            ...
            const versionResult = db.prepare(`
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
                `).run(newRecipeId, reply.description, reply.instructions, reply.ingredients, reply.source_prompt, reply.ai_model, reply.relation);
            ...
            const versionResult = db.prepare(`
                INSERT INTO recipe_versions (recipe_id, description, instructions, ingredients, source_prompt, ai_model, relation)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                `).run(recipeId, reply.description, reply.instructions, reply.ingredients, reply.source_prompt, reply.ai_model, reply.relation);
